...
static void fn1() {
    ...
    for (k = 0; k < NUM; k++) {
        ...
        if (temp_j > temp_i) {
            atomic_fetch_add_explicit(&count_j, 1, std::memory_order_relaxed);
        }
    }
...
static void fn2() {
    ...
    for (k = 0; k < NUM; k++) {
        ...
        if (temp_i > temp_j) {
            atomic_fetch_add_explicit(&count_i, 1, std::memory_order_relaxed);
        }
    }
...
static void check_assert() {
    ...
    int c_i = count_i.load(std::memory_order_relaxed);
    int c_j = count_j.load(std::memory_order_relaxed);
    if ((c_i == NUM and c_j == NUM - 1) or (c_i == NUM - 1 and c_j == NUM)) {
        if (!(i.load(std::memory_order_relaxed) == FIB_NUM or
              j.load(std::memory_order_relaxed) == FIB_NUM)) {
            assert_failed = true;
        }
    }
